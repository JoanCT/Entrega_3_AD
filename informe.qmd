...
```
Com podem observar, la previsió no és del tot bona, i de fet hem vist que és millor preveure la sèrie amb el model multiplicatiu.

Per tant, a partir del model multiplicatiu, predirem els valors de la sèrie temporal pels 12 trimestres següents. Per això, el que farem serà multiplicar la tendència amb l'estacionalitat. Recordem que la tendència l'aproximam amb una recta de regressió lineal (vista a l'inici de l'exercici). L'estacionalitat és periòdica, cada any és la mateixa, llavors el que farem serà repetir 3 vegades els 4 últims valors d'estacionalitat calculats anteriorment. El terme de renou no el tenim en compte per a la predicció, i per tant formarà part de l'error del model. Anem a calcular l'estimació, de manera similiar al model additiu:
...
Veim que la previsió segueix el patró de la sèrie, però s'equivoca en les magnituds dels pics. Tot i això, se veu a simple vista que l'aproxima millor que el model additiu.

Finalment, calculem l'error quadràtic mitjà (MSE) de l'aproximació:
```{r}
